...
// TODO: to show notifications

function showNotifications(e) {
  // e.preventDefault();
  var year = document.querySelector('#year_not');
  var semester = document.querySelector('#semester_not');
  var section = document.querySelector('#section_not');
  ...
  var urlString = "Notificationdownload.php?year="+selectedYear+"&semester="+selectedSemester+"&section="+selectedSection;

  if(selectedYear == "" && selectedSemester == "" && selectedSection == "") {
    document.querySelector('#showNotifications').innerHTML = '';
    return;
  }
    ...
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        console.log(this.responseText);
        document.getElementById("showNotifications").innerHTML = this.responseText;
      }
    };
    xmlhttp.open("GET",urlString,true);
    xmlhttp.send();
